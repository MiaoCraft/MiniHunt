...
public class ChangeRoleCommand {
    ...
    public static boolean changeRole(CommandSender sender, String arg, Game game) {
        // 只有在游戏未开始时才能改变角色
        if (!net.mcxk.minihunt.game.GameStatus.WAITING_PLAYERS.equals(game.getStatus()) || !(sender instanceof Player)) {
            SendMessage.sendMessage(String.format("%s游戏已开始！", ChatColor.RED), sender);
            return true;
        }
        Player player = (Player) sender;
        final net.mcxk.minihunt.game.PlayerRole playerRole = game.getIntentionRoleMapping().get(player);
        // 判断由观战转为游戏玩家时，如果游戏人数已经达到最大游戏人数，不能变更游戏模式
        if (net.mcxk.minihunt.game.PlayerRole.WAITING.equals(playerRole) &&
                !net.mcxk.minihunt.game.ConstantCommand.WAITING.equals(arg) &&
        ...
        switch (arg) {
            case net.mcxk.minihunt.game.ConstantCommand.HUNTER:
                game.getIntentionRoleMapping().put(player, net.mcxk.minihunt.game.PlayerRole.HUNTER);
                ...
                return true;
            case net.mcxk.minihunt.game.ConstantCommand.RUNNER:
                game.getIntentionRoleMapping().put(player, net.mcxk.minihunt.game.PlayerRole.RUNNER);
                game.getInGamePlayers().add(player);
                ...
                return true;
            case net.mcxk.minihunt.game.ConstantCommand.WAITING:
                game.getIntentionRoleMapping().put(player, net.mcxk.minihunt.game.PlayerRole.WAITING);
