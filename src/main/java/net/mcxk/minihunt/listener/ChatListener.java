...
public class ChatListener implements Listener {
    ...
    public void chat(AsyncPlayerChatEvent event) {
        ...
        Optional<net.mcxk.minihunt.game.PlayerRole> role = MiniHunt.getInstance().getGame().getPlayerRole(event.getPlayer());
        if (!role.isPresent()) {
        ...
        if (event.getMessage().startsWith("#")) {
            ...
            if (role.get() == net.mcxk.minihunt.game.PlayerRole.HUNTER) {
                GetPlayerAsRole.getPlayersAsRole(net.mcxk.minihunt.game.PlayerRole.HUNTER).forEach(p -> p.sendMessage(ChatColor.GRAY + "[TEAM] " + event.getPlayer().getDisplayName() + ": " + ChatColor.RESET + event.getMessage()));
            }
        } else {
            if (role.get() == net.mcxk.minihunt.game.PlayerRole.HUNTER) {
